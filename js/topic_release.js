	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
					width            : "688px",  					// 宽度
					height           : "auto",  					// 高度
					itemWidth        : "140px",                     // 照片项的宽度
					itemHeight       : "120px",                     // 照片项的高度
					url              : "/upload/UploadAction",  	// 上传照片的路径
					multiple         : true,  						// 是否可以多个照片上传
					dragDrop         : true,  						// 是否可以拖动上传照片
					del              : true,  						// 是否可以删除照片
					finishDel        : false,  						// 是否在上传照片完成后删除预览
					/* 提供给外部的接口方法 */
					onSelect         : function(selectFiles, files){},// 选择照片的回调方法  selectFile:当前选中的照片  allFiles:还没上传的全部照片
					onDelete		 : function(file, files){},     // 删除一个照片的回调方法 file:当前删除的照片  files:删除之后的照片
					onSuccess		 : function(file){},            // 照片上传成功的回调方法
					onFailure		 : function(file){},            // 照片上传失败的回调方法
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<div class="filePicker">点击选择照片</div>';
	            	html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area">或者将照片拖到此处</span>';
					html += '			</div>';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张照片，共0B。</div>';
		            html += '				<div class="btns">';
					...
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">确认上传照片</button>';
					html += '		</div>';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">选择照片</div>';
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div id="preview" class="upload_preview">';
				    html += '				<div class="add_upload">';
				    html += '					<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" title="点击添加照片" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
			...
			this.funSetStatusInfo = function(files){
				...
				// 转化为kb和MB格式。照片的名字、大小、类型都是可以现实出来。
				if (size > 710 * 710) {                    
					size = (Math.round(size * 100 / (710 * 710)) / 100).toString() + 'MB';                
				} else {                    
					size = (Math.round(size * 100 / 710) / 100).toString() + 'KB';                
				}  
			...
			this.funFilterEligibleFile = function(files){
				var arrFiles = [];  // 替换的照片数组
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 51200000) {
						alert('您这个"'+ file.name +'"照片大小过大');	
					} else {
						// 在这里需要判断当前所有照片中
						arrFiles.push(file);	
					}
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						var funDealtPreviewHtml = function() {
							...
							if (file) {
								...
								// 走到这里说明照片html已经组织完毕，要把html添加到预览区
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<p>上传成功，照片地址是：" + response + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的照片
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
					...
					onFailure: function(file) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						$("#uploadInf").append("<p>照片" + file.name + "上传失败！</p>");	
